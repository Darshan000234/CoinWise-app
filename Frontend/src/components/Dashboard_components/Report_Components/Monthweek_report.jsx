    ...
    const fetchData = async () => {
      try {
        ...
        const [barRes, pieRes, lineRes] = await Promise.all([
          axios.get(
            `${URL}/dashboard/report/bar?type=${barFilter.type}&range=${barFilter.range}`,
            { withCredentials: true }
          ),
          axios.get(
            `${URL}/dashboard/report/pie?type=${pieFilter.type}&range=${pieFilter.range}`,
            { withCredentials: true }
          ),
          axios.get(
            `${URL}/dashboard/report/line?type=${lineFilter.type}&range=${lineFilter.range}`,
            { withCredentials: true }
          ),
        ]);
      ...
      <div ref={chartRefs[2]} className="bg-[#111] p-6 rounded-xl shadow-lg">
        {lineData && lineData.dailyData && lineData.dailyData.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center h-96 text-gray-400">
            <Inbox size={48} className="mb-4" />
            <span className="text-lg font-medium">
              No daily transactions available
            </span>
          </div>
        )}
      </div>
